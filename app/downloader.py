...
def _format_selector(url: str) -> str:
    parsed = urllib.parse.urlparse(url)
    if "youtube.com" in parsed.netloc.lower() or "youtu.be" in parsed.netloc.lower():
        return "best[height<=720][acodec!=none]/best[height<=720]/best[acodec!=none]/best"
    if "tiktok.com" in parsed.netloc.lower():
        return "best[acodec!=none][vcodec^=h264]/download/best[acodec!=none]/bv*+ba/best"
    return "bv*+ba/b[acodec!=none]/best[acodec!=none]/best"
...
def download_video_url(
    *,
    ...
    def collect_finished(download: dict[str, Any]) -> None:
        if progress_callback:
            progress_callback({
                ...
                "downloaded_bytes": download.get("downloaded_bytes") or 0,
                "total_bytes": download.get("total_bytes") or download.get("total_bytes_estimate") or 0,
                "speed": download.get("speed") or 0,
    ...
    ydl_opts: dict[str, Any] = {
        "outtmpl": outtmpl,
        "format": _format_selector(cleaned_url),
