...
def find_binary(name: str) -> str:
    ...
    if env_value:
        ...
        if not configured.exists():
            raise RuntimeError(f"{env_name} 指向的文件不存在：{configured}")
        if _binary_works(configured):
            _BINARY_CACHE[name] = str(configured)
            return str(configured)
        raise RuntimeError(f"{env_name} 指向的 {name} 无法运行：{configured}")

    candidates = _candidate_binaries(name)
    ...
    failure_text = f" 已找到但无法运行：{', '.join(failures)}。" if failures else ""
    raise RuntimeError(
        f"找不到 {name}。请把 {name} 可执行文件放到 {target_dir}，"
        f"或先安装 FFmpeg 并确保命令可在终端中运行。{failure_text}"
    )
